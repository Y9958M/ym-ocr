...
flowchart LR
    ...
    subgraph YmOcr["ym-ocr 单进程 (uvicorn)"]
        ...
        Engine["PaddleOCR 单例<br/>PP-OCRv6 medium"]
        Rest --> Core
        Mcp --> Core
        Core --> Engine
    end
...
- `import paddle/paddlex` 前设 `PADDLE_PDX_DISABLE_MODEL_SOURCE_CHECK=True`、`FLAGS_use_mkldnn=0`（CPU 兼容，对应 Paddle 3.3 PIR/OneDNN 已知问题）
- `PaddleOCR(use_doc_orientation_classify=False, use_doc_unwarping=False, use_textline_orientation=False, device=OCR_DEVICE, precision="fp16")`
- 模型档位由 `OCR_MODEL` 控制（默认 `PP-OCRv6_medium`，可切 `small`/`tiny`）
...
- PDF：`fitz.open(stream)` → 逐页 `get_pixmap(matrix=2.0)` → `engine.predict`，合并结果，限 50 页
...
默认 `PP-OCRv6_medium`。建议先用 medium 跑招聘简历样本，记录显存占用与精度；不满足再切 `small`/`tiny`，仅改 `OCR_MODEL` 环境变量，代码不动。
